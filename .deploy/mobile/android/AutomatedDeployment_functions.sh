...
function PrepareAndroidSDK() {
  ...
  pushd ${ANDROID_PLACE} >/dev/null;
    ...
    pushd ~/Downloads >/dev/null;
      ...
      wget -nc http://dl-ssl.google.com/android/repository/${SDK_TOOLS_ZIP}
    popd >/dev/null;
...
function BuildAndroidAPK() {
  ...
  pushd ${TARGET_DIRECTORY}/android >/dev/null;
    ...
    mv ${PROJECT_NAME}.apk ${BUILD_DIRECTORY}/${APK_PUBLISH_DIR};
    echo -e "SDK Platform Android 6.0, API 23, revision 3 : " > ${BUILD_DIRECTORY}/${APK_PUBLISH_DIR}/${PROJECT_NAME}.apk.txt
